...
def get_catalogue():
    ...
    try:
        ...
        # Key Maestra (por si la usas en el conversor)
        if all(c in df.columns for c in ["Referencia", "Color", "Talla"]):
            df["KEY_MASTER"] = (
                df["Referencia"].astype(str).str.strip().str.upper() + "_" +
...
# ==========================================
# PESTAÑA 1: GESTIÓN DE PETICIONES
# ==========================================
with tab1:
    ...
    origen = c2.selectbox(
        ...
        [
            ...
            "PET T002 Marbella",
            "PET T004 Madrid",
            "PET Almacén Portugal",
        ],
    )
    destino = c3.selectbox(
        "DESTINO",
        [
            "PET Almacén Badalona",
            "PET Almacén Ibiza",
    ...
    # ✅ AVISO + BLOQUEO si ORIGEN == DESTINO
    ...
    if archivo_v and st.button(
        "CARGAR DATOS DEL EXCEL",
        type="primary",
        key="b1",
        disabled=same_wh,  # ✅ bloquea si mismo almacén
    ):
        ...
        # Normaliza catálogo (por si cached antiguo)
        df_cat["EAN"] = df_cat["EAN"].astype(str).str.replace(".0", "", regex=False).str.strip()
        ...
        for _, f_v in df_v.iterrows():
            ...
            if match.empty:
                ...
                if len(not_found_samples) < 10:
                    not_found_samples.append(ean_v)
                continue
        ...
        if added == 0:
            st.error(
                "❌ No se ha añadido ningún producto al carrito. "
                "Revisa que los EAN del Excel existan en el catálogo."
            )
        else:
            st.success(f"✅ {added} productos añadidos/actualizados en el carrito.")
    ...
    if st.session_state.carrito:
        ...
        with cB:
            # Ejemplo: bloquear “generar” si mismo almacén
            if st.button("GENERAR PETICIÓN", type="primary", disabled=same_wh):
                if same_wh:
                    ...
                    st.stop()
                st.success("Petición generada (placeholder). Aquí iría tu lógica de exportación a Excel / Gextia.")
    else:
        st.info("El carrito está vacío.")

# ==========================================
# PESTAÑA 2: CONVERSOR GEXTIA (placeholder)
# ==========================================
with tab2:
