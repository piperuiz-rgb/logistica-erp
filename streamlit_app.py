...
# =========================================================
    ...
    carrito = st.session_state.carrito.copy()
    ...
    for ean, item in carrito.items():
        ...
        with col2:
            ...
            carrito[ean]["qty"] = nueva_qty
        
        with col3:
            if st.button("🗑️", key=f"del_{ean}"):
    ...
    # Opción para añadir productos manualmente
    with st.expander("➕ Añadir producto manualmente"):
        st.info("Busca productos del catálogo para añadirlos a la petición")
        ...
        if busqueda:
            ...
            resultados = df_cat[mask].head(10)
            ...
            if not resultados.empty:
                for _, row in resultados.iterrows():
                    ...
                    with col2:
                        if st.button("Añadir", key=f"add_{row['EAN']}"):
                            ean = str(row['EAN'])
                            if ean in st.session_state.carrito:
                                st.session_state.carrito[ean]["qty"] += 1
                            else:
    ...
    total_unidades = sum(item["qty"] for item in st.session_state.carrito.values())
