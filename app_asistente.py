...
# Guard de flujo: si falta configuración (origen/destino), vuelve al Paso 1
cfg = st.session_state.get("config", {})
if not cfg.get("origen") or not cfg.get("destino"):
    st.session_state.step = 1
...
# =========================================================
    ...
    with st.expander("➕ Añadir producto manualmente", expanded=True):
        ...
        manual_limit = st.selectbox(
            ...
            index=[5, 10, 20, 50, 100].index(int(st.session_state.get("manual_limit", 10))) if int(st.session_state.get("manual_limit", 10)) in [5, 10, 20, 50, 100] else 1,
            key="manual_limit",
        )
